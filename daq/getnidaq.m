function [s] = getnidaq(varargin)
%% getnidaq.m USAGE NOTES AND CREDITS
% 
% Syntax
% -----------------------------------------------------
%     nidaq(vrts,tets)
...
if nargin > 0
    ...
    s = daq.createSession(nidaq.vendor);

    % Make ready the two output channels
    addAnalogOutputChannel(s,nidaq.device,nidaq.channelOut,nidaq.outtype)
    addAnalogOutputChannel(s,nidaq.device,nidaq.channelIn,nidaq.outtype)

    % You can change the output rate like so...
    s.Rate = nidaq.rate; % an output rate of 8000 makes sense
...
outputSignal3 = pulsePatternPerSec;
